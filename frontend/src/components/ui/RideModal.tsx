      ...
      >
        ...
        <div className="relative h-52 sm:h-64 flex-shrink-0 rounded-t-2xl overflow-hidden">
          ...
          <div className="absolute top-4 left-4 flex flex-wrap gap-2 z-20">
            <span
              className={`inline-flex items-center text-[0.6rem] font-semibold tracking-[0.12em] uppercase px-3 py-1 rounded-full bg-black/60 backdrop-blur-sm border border-current ${routeTypeClass[ride.routeType]}`}
            >
              {ride.routeType}
            </span>
            {ride.past && (
          ...
          <div className="absolute inset-x-0 bottom-0 px-5 pb-4 z-20 flex items-end flex-wrap justify-between gap-3">
            <div className="min-w-0">
              <p className="font-[var(--font-accent)] text-[var(--color-accent)] text-xs lg:text-sm font-semibold tracking-wide flex items-center gap-1.5 mb-1">
                {localRide.location.from} <span className="opacity-50">→</span>{" "}
          ...
          {userDetails?.role !== "rider" && localRide.riders && (
            <div className="mt-4 pt-4 border-t border-[var(--color-border)]/20">
              ...
              {(() => {
                ...
                const filteredRiders = localRide.riders.filter((r) =>
                  r.username.toLowerCase().includes(debouncedQuery.toLowerCase())
                );

                if (filteredRiders.length === 0) {
                  return (
                    <p className="text-sm text-secondary italic">
                      No riders found matching "{debouncedQuery}".
                    </p>
                  );
                }
