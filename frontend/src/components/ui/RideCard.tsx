...
export default function RideCard({
  ...
  const [isConfirmWithdrawOpen, setIsConfirmWithdrawOpen] =
    useState<boolean>(false);
  const [isAttendanceProcessing, setIsAttendanceProcessing] =
    useState<boolean>(false);
  ...
  const executeWithdrawRide = async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(err) && err.response) {
        showError(
          err.response.data.message || "Failed to withdraw from the ride.",
        );
      } else if (err instanceof Error) {
        showError(err.message);
      } else {
              ...
              <div className="relative h-52 sm:h-64 flex-shrink-0 rounded-t-2xl overflow-hidden">
                ...
                <div className="absolute inset-x-0 bottom-0 px-5 pb-4 z-20 flex items-end justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <h2 className="font-[var(--font-heading)] font-black text-2xl sm:text-3xl text-white leading-tight">
                      {localRide.title}
                    </h2>
                  </div>
                  {!localRide.past ? (
              ...
              <div className="p-5 sm:p-8 flex flex-col gap-6">
                ...
                {userDetails?.role !== "rider" && localRide.riders && (
                  <div className="mt-8 pt-6 border-t border-[var(--color-border)]/20">
                    <h4 className="font-[var(--font-heading)] text-lg text-white mb-4">
                      Enrolled Riders
                    </h4>
