...
export default function ProfileEditModal({ profile, onClose, onSave }: ProfileEditModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updates: Partial<Profile> = {
        ...
        bike: formData.bike.split(",").map((s) => s.trim()).filter(Boolean),
  ...
  const getUsernameHelperText = (): { text: string; color: string } | null => {
    if (usernameStatus === "invalid") {
      return { text: "3–30 chars, lowercase letters, numbers, and underscores only.", color: "text-red-400" };
    }
        ...
        >
          ...
          <div className="shrink-0 p-6 sm:p-8 pb-5 border-b border-white/5 flex justify-between items-start relative z-10 bg-gradient-to-b from-white/[0.02] to-transparent">
            <div>
              ...
              <p className="text-white/50 text-sm mt-1.5 font-body">Update your identity, garage details, and social links.</p>
            </div>
            <button 
              onClick={onClose}
              disabled={loading}
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-6 sm:p-8">
            <form id="profile-edit-form" onSubmit={handleSubmit} className="flex flex-col gap-10">
              ...
              <div className="flex flex-col gap-6">
                <div className="flex items-center gap-3 border-b border-white/10 pb-3">
                  ...
                  <h3 className="font-heading font-bold text-lg text-white uppercase tracking-widest">Basic Identity</h3>
                </div>
                  ...
                  <div className="sm:col-span-2 flex flex-col gap-2">
                    ...
                    <div className="relative">
                      ...
                      <div className="absolute top-1/2 right-4 transform -translate-y-1/2">
                        {usernameStatus === "checking" && <Cliploader size={14} color="var(--color-accent)" />}
                        {usernameStatus === "available" && <span className="text-green-400 font-bold">✓</span>}
                        {(usernameStatus === "taken" || usernameStatus === "invalid") && <span className="text-red-400 font-bold">✕</span>}
                      </div>
                    </div>
                    {helperText && (
                      <p className={`text-xs ml-1 font-body ${helperText.color}`}>{helperText.text}</p>
                    )}
                  </div>

                  <div className="flex flex-col gap-2">
                    <label htmlFor="name" className={labelClass}>Display Name</label>
                    <input id="name" type="text" name="name" autoComplete="name" value={formData.name} onChange={handleChange} className={inputClass} placeholder="John Doe" />
                  </div>

                  <div className="flex flex-col gap-2">
                    <label htmlFor="headline" className={labelClass}>Headline</label>
                    <input id="headline" type="text" name="headline" autoComplete="organization-title" value={formData.headline} onChange={handleChange} className={inputClass} placeholder="e.g. Stunt Rider" />
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-6">
                <div className="flex items-center gap-3 border-b border-white/10 pb-3">
                  <div className="w-8 h-8 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="m12 18-7 4 2-7-5-5 7-1 3-7 3 7 7 1-5 5 2 7-7-4z"/></svg>
                  </div>
                  <h3 className="font-heading font-bold text-lg text-white uppercase tracking-widest">Rider Details</h3>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div className="flex flex-col gap-2">
                    <label htmlFor="location" className={labelClass}>Location</label>
                    <input id="location" type="text" name="location" autoComplete="street-address" value={formData.location} onChange={handleChange} className={inputClass} placeholder="City, Country" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label htmlFor="years" className={labelClass}>Years Riding</label>
                    <input id="years" type="number" name="years" autoComplete="off" value={formData.years} onChange={handleChange} min="0" className={inputClass} placeholder="e.g. 5" />
                  </div>
                </div>

                <div className="flex flex-col gap-2">
                  <label htmlFor="bike" className={labelClass}>Bikes (comma separated)</label>
                  <input id="bike" type="text" name="bike" autoComplete="off" value={formData.bike} onChange={handleChange} className={inputClass} placeholder="e.g. Yamaha R1, Kawasaki Ninja" />
                </div>
              </div>
              ...
              <div className="flex flex-col gap-6">
                <div className="flex items-center gap-3 border-b border-white/10 pb-3">
                  ...
                  <h3 className="font-heading font-bold text-lg text-white uppercase tracking-widest">Social Links</h3>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                  <div className="flex flex-col gap-2">
                    <label htmlFor="instagram" className={labelClass}>Instagram</label>
                    <input id="instagram" type="text" name="instagram" autoComplete="url" value={formData.instagram} onChange={handleChange} className={`${inputClass} text-sm`} placeholder="URL" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label htmlFor="youtube" className={labelClass}>YouTube</label>
                    <input id="youtube" type="text" name="youtube" autoComplete="url" value={formData.youtube} onChange={handleChange} className={`${inputClass} text-sm`} placeholder="URL" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label htmlFor="facebook" className={labelClass}>Facebook</label>
                    <input id="facebook" type="text" name="facebook" autoComplete="url" value={formData.facebook} onChange={handleChange} className={`${inputClass} text-sm`} placeholder="URL" />
                  </div>
                </div>
              </div>
