    ...
    <section className="py-12 lg:py-22 relative overflow-hidden bg-gradient-to-b from-[var(--color-bg)] via-[var(--color-section)] to-black border-t border-[var(--color-border)]/20">
      ...
      >
        ...
        <motion.span
          ...
          className="inline-flex items-center gap-2 text-[0.65rem] font-semibold tracking-[0.2em] uppercase px-5 py-2 rounded-full border border-[var(--color-border)]/60 text-[var(--color-accent)] bg-[var(--color-accent)]/10 mb-10"
        >
        ...
        >
          Apply to join MotoXCode and ride with people who treat the road as
          sacred. No posers. Just riders.
        </motion.p>
        ...
        >
          {!isInitialized ? null : userDetails ? (
            <Link
              ...
              className="btn-primary px-6 md:px-10 py-4 text-sm"
            >
              View My Profile
            </Link>
          ) : (
          ...
          <Link
            to="/rides"
            className="btn-secondary px-6 md:px-10 py-4 text-sm"
          >
        ...
        >
          {statsData.map((stat) => (
            <div key={stat._id} className="flex flex-col items-center gap-1">
              <span className="font-heading font-black text-3xl text-[var(--color-primary)] leading-none">
                {stat.target}
                {stat.suffix}
              </span>
              <span className="font-accent text-[0.65rem] tracking-[0.18em] text-[var(--color-text-secondary)] uppercase">
                {stat.label}
              </span>
            </div>
          ))}
